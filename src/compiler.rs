    ...
    loop {
        ...
        for (full_path, function) in symbols.functions.iter().filter(|x| x.1.is_generic()) {
            ...
            for x in &gi {
                ...
                compile_function_body(function, &llvm_struct_name, symbols, output)?;
        ...
        for (name, r#struct) in symbols.types.iter().filter(|x| x.1.is_generic()) {
            ...
            for x in &gi {
                ...
                let llvm_struct_name = format!("{}<{}>", full_path, x.iter().map(|x| get_llvm_type_str(&substitute_generic_type(x, &type_map), symbols, &current_path)).collect::<CompileResult<Vec<_>>>()?.join(", "));
...
fn qualify_type(ty: &ParserType, current_path: &str, symbols: &SymbolTable) -> ParserType {
    ...
    match ty {
        ParserType::Named(_) | ParserType::Generic(_, _) => {
            
            if symbols.get_type(&ty.to_string()).is_ok()  {
                return ty.clone();
            }
            else {
...
pub fn compile_statement(stmt: &Stmt, ctx: &mut CodeGenContext, output: &mut LLVMOutputHandler) -> CompileResult<()>{
    match &stmt {
        Stmt::Let(name, var_type, expr) => {
            ...
            ctx.scope.add_variable(name.clone(), Variable::new(substitute_generic_type(var_type, &ctx.symbols.alias_types), false), x);
            // output.push_str(&format!("    %v{} = alloca {}; var: {}\n", x, llvm_type, name));
...
pub fn get_llvm_type_str(
    ...
    match ptype {
        ...
        ParserType::NamespaceLink(_, c) => {
            ...
            if let Ok(x) = symbols.get_type(&fqn) {
                if x.is_generic() {
        ...
        ParserType::Generic(_, imp) => {
            ...
            if let Some(x) = symbols.types.get(&abs) {
                ...
                return Ok(format!("%\"struct.{}<{}>\"", abs, imp.iter().map(|x| get_llvm_type_str(x, symbols, current_namespace)).collect::<CompileResult<Vec<_>>>()?.join(", ")));
            }
            if let Some(x) = symbols.types.get(&rel) {
                x.generic_implementations.borrow_mut().push(imp.clone());
                return Ok(format!("%\"struct.{}<{}>\"", rel, imp.iter().map(|x| get_llvm_type_str(x, symbols, current_namespace)).collect::<CompileResult<Vec<_>>>()?.join(", ")));
            }
        }
    }
